...
color = [["red", "blue"][i] for i in y.reshape(-1, )]
plt.scatter(x[:, 0], x[: , 1], c= color, alpha= 0.4, s= 90)
plt.show()
...
bo = np.random.normal(size= (1, n_output))
parameters = [wo, bo]
...
def predict(x, proba= True):
    y_hat = sigmoid(x @ parameters[0] + parameters[1])
    if proba: return y_hat
    else: return (y_hat > 0.5) * 1.0

def train(x, y, x_val, y_val, epochs, batch_size):
    train_loss = [bce(y, predict(x, proba= True))]
    valid_loss = [bce(y_val, predict(x_val, proba= True))]
    ...
    for epoch in range(epochs):
        ...
        train_loss.append(bce(y, predict(x, proba=True)))
        valid_loss.append(bce(y_val, predict(x_val, proba=True)))
...
plt.plot(val_loss, c= "blue", label= "val loss")
plt.legend()
plt.show()
