import numpy as np
from keras.layers import Layer, Input, Dense, TimeDistributed
...
class MyGRU(Layer):
    ...
    def build(self, input_shape):
        ...
        self.wu = self.add_weight(shape= (input_shape[-1], self.units))
        self.wc = self.add_weight(shape= (input_shape[-1], self.units))

        # weights for h
        self.hr = self.add_weight(shape= (self.units, self.units))
        self.hu = self.add_weight(shape= (self.units, self.units))
        self.hc = self.add_weight(shape= (self.units, self.units))

        # bias
        self.br = self.add_weight(shape= (self.units, ))
        self.bu = self.add_weight(shape= (self.units, ))
        self.bc = self.add_weight(shape= (self.units, ))

    def call(self, x):
        def compute(x, h):
            rt = tf.math.sigmoid((x @ self.wr) + (h @ self.hr) + self.br)
            zt = tf.math.sigmoid((x @ self.wu) + (h @ self.hu) + self.bu)
            gt = tf.math.tanh((x @ self.wc) + ((rt * h) @ self.hc) + self.bc)
            ht = (h * (1 - zt)) + (zt * gt)
            return ht
        ...
        if self.return_sequences:
            h = tf.convert_to_tensor(h) # shape = (20, None, 50)
            h = tf.transpose(h, perm= (1, 0, 2)) # shape = (None, 20, 50)
            return h
        else: return h[-1]
...
y_train = np.array([data[(i + 1): (i + 1 +  n_step)] for i in range(n - n_step)])
...
hidden = MyGRU(units= n_hidden, return_sequences= True)(x_input)
y_output = TimeDistributed(Dense(units= n_output))(hidden)
