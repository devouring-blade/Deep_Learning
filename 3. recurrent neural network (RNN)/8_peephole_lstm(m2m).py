import numpy as np
from keras.layers import Layer, Dense, Input, TimeDistributed
...
class MyPeepholeLSTM(Layer):
    ...
    def call(self, x):
        ...
        for t in range(x.shape[1]):
            ct, ht = compute(x[: , t, : ], c[-1], h[-1])
            c.append(ct), h.append(ht)
        c.pop(0), h.pop(0)
...
hidden = MyPeepholeLSTM(units= n_hidden, return_sequences= True)(x_input)
