...
# ==================== ENCODER ==================== #
class Encoder(Layer):
    def __init__(self, num_layer, seq_len, num_feat, num_head, num_ff, dropout_rate=0.1):
        super().__init__()
        self.pos_encoding = PositionalEncoding(seq_len, num_feat)
        self.enc_layers = [EncoderLayer(num_feat, num_head, num_ff, dropout_rate) for _ in range(num_layer)]
...
class Decoder(Layer):
    def __init__(self, num_layer, seq_len, num_feat, num_head, num_ff, dropout_rate=0.1):
        super().__init__()
        self.pos_encoding = PositionalEncoding(seq_len, num_feat)
        self.dec_layers = [DecoderLayer(num_feat, num_head, num_ff, dropout_rate) for _ in range(num_layer)]
