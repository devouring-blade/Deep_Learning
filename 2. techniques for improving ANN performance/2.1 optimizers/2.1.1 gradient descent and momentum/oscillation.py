...
wo = np.array([0.1, 3], dtype= "float32")
wt = np.array([3, 2], dtype= "float32")
w = tf.Variable(wo)

# opt = optimizers.SGD(learning_rate= 0.8, momentum= 0.0)     # no momentum
opt = optimizers.SGD(learning_rate= 0.8, momentum= 0.2)   # use momentum
...
for i in range(80):
    ...
    with tf.GradientTape() as tape:
        # the gradient of w1-axis is small and the gradient of w2-axis is large
